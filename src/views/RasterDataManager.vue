<template>
  <div class="raster-manager-container">
    <!-- 数据表格 -->
...
// 表格相关
const rasterTableRef = ref(null);
const loading = ref(false);
const searchQuery = ref('');
const selectedDataIds = ref([]);
const currentPage = ref(1);
const pageSize = ref(20);
const totalRasterData = ref(0);
...
// 过滤后的数据
const filteredRasterData = computed(() => {
  // 确保dataStore.rasterData存在
  if (!dataStore.rasterData) {
    return [];
  }
  
  const query = searchQuery.value.toLowerCase();
  if (!query) {
    // 使用rasterData而不是localDataList
    return dataStore.rasterData;
  }
  
  // 使用rasterData而不是localDataList
  return dataStore.rasterData.filter(item => 
...
const refreshData = async () => {
  loading.value = true;
  try {
    console.log('正在刷新栅格数据...');
    
    // 调用数据存储的fetchRasterData方法获取数据
    const data = await dataStore.fetchRasterData();
    
    // 确保数据被正确赋值
    if (Array.isArray(data) && data.length > 0) {
      console.log(`获取到 ${data.length} 条栅格数据`);
      totalRasterData.value = data.length;
    } else {
      console.log('未获取到栅格数据或数据为空');
      
      // 尝试使用模拟数据（仅开发时使用）
      if (!dataStore.rasterData || dataStore.rasterData.length === 0) {
        console.log('生成测试数据...');
        // 生成一些测试数据
        for (let i = 1; i <= 5; i++) {
          dataStore.addLocalData({
            ...
            name: `栅格数据 ${i}`,
            type: 'raster',
            rasterType: i % 2 === 0 ? 'GeoTIFF' : 'JPEG',
            bands: Math.floor(Math.random() * 5) + 1,
            resolution: `${i * 5}m`,
        ...
        // 尝试再次获取数据
        await dataStore.fetchRasterData();
        totalRasterData.value = dataStore.rasterData?.length || 0;
      }
    }
...
const handleRowClick = (row) => {
  rasterTableRef.value.toggleRowSelection(row);
};
...
const handleDelete = async (row) => {
  try {
    ...
    try {
      // 使用实际的API删除
      const response = await dataStore.deleteRasterData(row.id);
      ...
      console.error('删除栅格数据失败:', apiError);
...
const handleBatchDelete = async () => {
  ...
  try {
    ...
    try {
      // 使用实际的API批量删除
      const response = await dataStore.batchDeleteRasterData(selectedDataIds.value);
...
const handleImportSuccess = (data) => {
  ...
  ElMessage.success(`成功导入 ${successCount} 条栅格数据`);
};
